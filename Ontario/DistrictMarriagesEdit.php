...
{			// perform update
	if ($debug)
	    $warn	.= "<p>count(\$_POST)=" . count($_POST);
	$record		= null;
	$domain		= null;
	$volume		= null;
	$reportNo	= null;
...
$crumbs	= array(
			...
			'/Canada/genProvince.php?Domain=CAON'
								=> 'Ontario',
			"/Ontario/DistrictMarriageEditQuery.php"
								=> 'District Marriage Query');
if ($volume)
{
	$title	.= "Volume $volume ";
...
if ($reportNo)
{
	...
	$crumbs["DistrictMarriagesEdit.php?Domain=$domain&Volume=$volume&ReportNo=$reportNoText"]	= 'Report';
}
if ($itemNo)
	$title	.= "Item $itemNo ";
if ($create)
...
<div class="body">
  <h1>
<span class="right">
	<a href="DistrictMarriagesEditHelpen.html" target="help">? Help</a>
</span>
	<?php print $title; ?>
...
<form name="countyForm" id="countyForm"
	action="DistrictMarriagesEdit.php" 
	method="post" 
	autocomplete="off" 
...
<?php
	    ...
	    foreach($reports as $record)
	    {
			while(true)
			{
			    ...
			    if ($reportNo == floor($reportNo))
					$reportNo	= floor($reportNo); 
			    else
			    if (($reportNo - floor($reportNo)) == 0.5)
					$reportNo	= floor($reportNo) . '½';
...
?>
	...
	<td class="center">
	    <button type="button" class="button" 
			id="Details<?php print $row; ?>" <?php print $disabled; ?>>
			Details
	    </button> 
	</td>
	<td class="center">
