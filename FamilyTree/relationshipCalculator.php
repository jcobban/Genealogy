...
$template           = new FtTemplate("relationshipCalculator$lang.html");
...
$template->set('NAME1',         $name1);
$template->set('NAME2',         $name2);
...
{       // display relationship
    $template->set('GENDERCLASS1',      $genderClass[$gender1]);
    $template->set('RELATION',          $relation);
    $template->set('GENDERCLASS2',      $genderClass[$gender2]); 
    if (!is_null($commonAncestor))
    {
        $namec  = $commonAncestor->getName(Person::NAME_INCLUDE_DATES);
        $template->set('NAMEC',         $namec);
        $template->set('COMMONIDIR',    $commonAncestor->getIdir());
        $genderClassAnc     = $genderClass[$commonAncestor->getGender()]; 
        $template->set('GENDERCLASSANC',$genderClassAnc); 
    }
    else
        $template['commonAncestor']->update(null);
}       // display relationship
else
    $template['relationship']->update(null);
