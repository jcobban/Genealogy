...
function censusCitation($year, $page, $idir, $type)
{
    ...
    {	                // citation to birth
		...
		{
		    if ($debug)
				print "<matches>" . print_r($matches, true) . "</matches>\n";
		    ...
		    if ($debug)
		    {
				print "<prov>$prov</prov>\n";
				print "<dist>$dist</dist>\n";
				print "<subdist>$subdist</subdist>\n";
				print "<div>$div</div>\n";
		...
		{	                // have at least one name to check
		    ...
		    {	            // select succeeded
				...
				{	        // found match in census
				    ...
				    print "	<surname>$csurname</surname>\n";
				    print "	<givennames>$cgivennames</givennames>\n";
				    print "    </row>\n";
 ...
 ************************************************************************/
    ...
    print "  <parms>\n";
    error_log("addCitXml.php:\n",3,$document_root . "/logs/addCitXml.txt");
    foreach ($_POST as $key => $value)
    {			    // look at all parameters
		$value		= trim($value);
		print "    <$key>" . xmlentities($value);
    error_log("addCitXml.php: $key='$value'\n",3,$document_root . "/logs/addCitXml.txt");
		switch($key)
		{		    // act on keys
		    case 'idsr':
