 ...
 *      toidir          an administrator can specify the toidir         *
 *                      parameter to delete all unattached Persons      *
 *                      between idir and toidir inclusive               *
 *                                                                      *
 *  History:                                                            *
 *      2010/12/20      created                                         *
 ...
 *      2022/04/19      add toidir parameter for administrator          *
 *                                                                      *
...
$idirtext           = null;
$toidir             = null;     // highest key of record to delete
$toidirtext         = null;
...
{                   // invoked by method=get
    ...
    foreach($_GET as $key => $value)
    {
        ...
        {           // act on specific parameter    
            ...
            case 'toidir':
            {       // highest key of instance of Person
                if (ctype_digit($value))
                {   // integer value
                    $toidir     = intval($value);
                    if ($toidir < $idir)
                        $msg    .= "TOIDIR must be greater than $idir. ";
                }   // integer value
                else
                    $toidirtext   = htmlspecialchars($value);
                break;
            }       // highest key of instance of Person
...
{                       // get the requested person
    if (is_null($toidir) || !canUser('yes'))
        $toidir             = $idir;
    for(; $idir <= $toidir; ++$idir)
    {                   // delete
        ...
        {                   // person is defined
            ...
            if ($person->isOwner())
            {
                ...
                {       // person is not connected to any others
                    if ($toidir == $idir)
                        $msg        .= "$name is a member of a family. Detach from parents, spouse, and children before trying to delete. ";
                }       // person is not connected to any others
            }
            else
            {           // user is not authorized to edit this record
                if ($toidir == $idir)
                    $msg    .= "Current user is not authorized to delete $name.  Contact the administrator or an existing owner of this family for permission.";
            }           // user is not authorized to edit this record
    
            if (strlen($msg) == 0)
            {       // OK to delete
                $counts         = $person->delete(false);
                $warn           .= "<p>Deleted $name: $idir</p>\n";
...
{                   // error
    $name       = "Missing IDIR value";
    $msg        .= "Missing IDIR value";
    $idir       = 0;
}                   // error
