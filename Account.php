...
{                   // request to change e-mail address
    if (strpos($email, "'") !== false ||
        strpos($email, '@') == false)
        $msg        .= $template['badEmail']->replace('$email', $email);
    else
...
if (strlen($cellphone) > 0 && $cellphone != $oldcellphone)
{                   // request to change mobile phone number
    if (preg_match('/[-+0-9 ()]*/', $cellphone))
    {
    }
    else
    {
        $msg        .= $template['badcellphone']->replace('$cellphone',$cellphone);
        $cellphone  = '';
    }
}                   // request to change mobile phone number
else
...
if ($useText)
    $options        = $options | User::OPT_USETEXT_ON;
...
$template->set('CELLPHONE',     $cellphone);
...
$template->set('chkusetext',    $chkusetext);
