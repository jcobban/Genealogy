...
import {getOffsetLeft, getOffsetTop, popupAlert, 
        eltMouseOver, eltMouseOut, displayHelp, hideHelp}
 ...
 ************************************************************************/
export function gotNicknames(obj)
{
...
export function capitalize(element)
{
    ...
    let needCap         = true;     // capitalize 1st letter
    //let msg             = "";
 ...
 ************************************************************************/
export function setErrorFlag(element, valid)
{
    let className   = element.className;
    // clear or set the error indicator if required by changing class name
    let errpos      = className.indexOf(' error');
    if (errpos >= 0)
 ...
 ************************************************************************/
export function expAbbr(element, table)
{
 ...
 ************************************************************************/
export function changeElt(element)
{
    let     id;
    if (element.form)
...
{
    let element         = this;
    let re      = /^([A-Za-z]*)\s*([0-9]*)\s*([A-Za-z]*)\s*([0-9]*)/;
...
{
    let element     = this;
    let cell        = this.parentNode;  // table cell
    let col     = cell.cellIndex;   // column number
    let row     = cell.parentNode;  // table row
    let rowIndex    = row.sectionRowIndex;  // position in body section
    let section     = row.parentNode;   // table section
    let prevRow;
    let prevCell;
    let prevFamily  = null;
    let expFamily   = null;
...
{
    let element     = this;
    let re      = new RegExp("^(http://|https://|ftp://|)([-A-Za-z0-9_.]*)/(.*/|)(.+)$");
...
{
    let re  = /([A-Za-z]*) *([0-9]*) *([A-Za-z]*) *([0-9]*)/;
    let result  = re.exec(date);
    let matched = typeof result === 'object' && result instanceof Array;
    let di  = 15;
    let mi  = 6;
    let yi  = -9999;
 ...
 ************************************************************************/
export function getCellRelRow(curr,
                              rel)
{
    let td;             // table cell containing input element
    let col;            // current column index
    let tr;             // table row containing input element
    //let row;          // current row index
    let field;          // returned value
    ...
    do
    {
        ...
        // wrap column number if it is outside the range of columns
        while(col < 0)
            col += tr.cells.length;
        ...
        rel         = rel / Math.abs(rel);
    }       // exit only when field is an <input type='text'>
    while(!field || !field.type || field.type != "text");
 ...
 *      any error in the input parameters, the current cell is returned.*
 ************************************************************************/
export function getCellFirstRow(curr)
{
    let td;         // table cell containing input element
    let col;        // current column index
    let tr;         // table row containing input element
 ...
 *      any error in the input parameters, the current cell is returned.*
 ************************************************************************/
export function getCellLastRow(curr)
{
    let td;         // table cell containing input element
    let col;        // current column index
    let tr;         // table row containing input element
 ...
 *      any error in the input parameters, the current cell is returned.*
 ************************************************************************/
export function getCellRelCol( curr,
                        rel)
{
    let td;         // table cell containing input element
    let col;        // current column index
    let row;        // current row index
    let tr;         // table row containing input element
    let tb;         // body section containing this row
    let field;      // returned value
    ...
    let msg     = "rel=" + rel + ", td.cellIndex=" + col + ", tr.rowIndex=" + row;
 ...
 ************************************************************************/
export function tableKeyDown(ev)
{
    let code                    = ev.key;

    // identify the column name and row number of the input element
    let colName                 = this.name;
    if (colName.length == 0)
        colName                 = this.id;
    let matches                 = /([a-zA-Z#_]+)(\d*)/.exec(colName);
    colName                     = matches[1];
    //let rowNum                      = matches[2];
 ...
 *      ev          a 'keydown' Event                                   *
 ************************************************************************/
export function numericKeyDown(ev)
{
    let key             = ev.key;
    if (/\d/.test(key))
        return true;
 ...
 *      ev          instance of 'click' Event                           *
 ************************************************************************/
export function columnClick(ev)
{
    ev.stopPropagation();
    let colIndex    = this.cellIndex;
    let row         = this.parentNode;
    let section     = row.parentNode;
    let table       = section.parentNode;
    let body        = table.tBodies[0];
    let footer      = table.tFoot;
    let footerRow   = null;
    if (footer)
 ...
 ************************************************************************/
export function linkMouseOver(ev)
{
 ...
 *      ev              instance of 'mouse' event                       *
 ************************************************************************/
export function linkMouseOut(ev)
{
    eltMouseOut.call(this, ev);
}       // function linkMouseOut
